...
export const db = {
  ...
  async updateProfile(userId, updates) {
    if (isSupabaseConfigured()) {
      const { data, error} = await (supabaseAdmin || supabase).from('profiles').update(updates).eq('id', userId).select().single();
      if (error) throw error;
      ...
      if (idx !== -1) {
        dbData.profiles[idx] = { ...dbData.profiles[idx], ...updates };
        writeMockDB(dbData);
        return true;
      }
      return false;
  ...
  async importStudentsCSV(studentsList, skipDuplicates = false) {
    const indexNumbers = studentsList.map(s => s.index_number);

    if (isSupabaseConfigured()) {
      const adminOrAnon = getSupabaseAdmin() || supabase;

      // Always fetch existing index numbers to differentiate inserts vs updates
      ...
      // When skipping duplicates, only insert brand-new students
      const toProcess = skipDuplicates
        ? studentsList.filter(s => !existingIndexNumbers.includes(s.index_number))
        : studentsList;

      const skippedDuplicates = skipDuplicates ? existingIndexNumbers.length : 0;

      if (toProcess.length === 0) {
        return { data: [], insertedCount: 0, updatedCount: 0, skippedDuplicates };
      }

      const { data, error } = await adminOrAnon
        .from('students')
        .upsert(toProcess, { onConflict: 'index_number' })
        .select();
      if (error) throw error;

      const updatedCount = skipDuplicates ? 0 : existingIndexNumbers.length;
      const insertedCount = skipDuplicates
        ? data.length
        : Math.max(0, data.length - updatedCount);

      return { data, insertedCount, updatedCount, skippedDuplicates };
    } else {
      const dbData = readMockDB();
      let insertedCount = 0;
      let updatedCount = 0;
      ...
      writeMockDB(dbData);
      return { data: studentsList, insertedCount, updatedCount, skippedDuplicates };
    }
  },
