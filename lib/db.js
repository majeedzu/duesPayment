...
export const db = {
  ...
  async createProfile(profile) {
    if (isSupabaseConfigured()) {
      const { data, error } = await (supabaseAdmin || supabase).from('profiles').insert(profile).select().single();
      if (error) throw error;
      return data;
    } else {
      const dbData = readMockDB();
  ...
  async importStudentsCSV(studentsList) {
    if (isSupabaseConfigured()) {
      ...
      const { data, error } = await (supabaseAdmin || supabase).from('students').upsert(studentsList, { onConflict: 'index_number' }).select();
      if (error) throw error;
      return data;
    } else {
      const dbData = readMockDB();
